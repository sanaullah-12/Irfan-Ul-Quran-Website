  ...
  const fetchData = async () => {
    try {
      ...
      if (overviewRes.status === "fulfilled") {
        ...
        setUserInfo(overviewRes.value.data.user);
      }
      if (coursesRes.status === "fulfilled")
        setCourses(coursesRes.value.data.courses || []);
      if (upcomingRes.status === "fulfilled")
        setUpcomingClasses(upcomingRes.value.data.classes || []);
      if (historyRes.status === "fulfilled")
        setClassHistory(historyRes.value.data.classes || []);
      if (nextRes.status === "fulfilled")
        setNextClass(nextRes.value.data.nextClass || null);
  ...
  if (authLoading || loading)
    return (
      <DashboardLayout
        ...
        pendingBadge={unreadCount > 0 ? unreadCount : undefined}
      >
    ...
    >
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Welcome back, {user.name}. Track your learning progress.
        </p>
      </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-600 dark:text-amber-500 mt-0.5">
                Request access to unlock Quran reader, Juz viewer, and learning
                materials.
              </p>
            </div>
          </div>
              ...
              {nextClass ? (
                <div>
                  ...
                  <div className="bg-slate-50 dark:bg-dark-bg rounded-xl p-4 border border-slate-200 dark:border-dark-border">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                      {fmt(nextClass.scheduledDate)} at{" "}
                      {fmtTime(nextClass.scheduledDate)} &middot;{" "}
                      {nextClass.duration}min &middot; {nextClass.teacherName}
                    </p>
                    <button
                      onClick={() =>
                        router.push(`/class-room/${nextClass.roomId}`)
                      }
                ...
                <EmptyState
                  ...
                  description="Your teacher will schedule your next class."
                />
              )}
            </div>
            ...
            <div className="bg-white dark:bg-dark-card rounded-xl border border-slate-100 dark:border-dark-border shadow-sm p-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold ${userInfo?.plan === "premium" ? "bg-gradient-to-r from-amber-400 to-yellow-500 text-white" : userInfo?.plan === "standard" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400"}`}
                  >
              ...
              <div className="space-y-2">
                {notifications
                  ...
                  .map((n) => (
                    <div
                      key={n._id}
                      className="flex items-start gap-3 p-3 rounded-xl bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 cursor-pointer"
                      onClick={() => {
                        markRead(n._id);
            ...
            <div className="bg-white dark:bg-dark-card rounded-xl border border-slate-100 dark:border-dark-border shadow-sm p-6">
              ...
              <div className="space-y-3">
                {upcomingClasses.slice(0, 3).map((cls) => (
                  <div
                    key={cls._id}
                    className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-dark-bg border border-slate-200 dark:border-dark-border"
                  >
                    <div>
                      <p className="text-sm font-medium text-slate-800 dark:text-white">
                        {cls.courseType} &middot; {cls.teacherName}
                      </p>
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        {fmt(cls.scheduledDate)} at {fmtTime(cls.scheduledDate)}{" "}
                        &middot; {cls.duration}min
                      </p>
                    </div>
                    <button
      ...
      {activeTab === "courses" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <div className="bg-white dark:bg-dark-card rounded-xl border border-slate-100 dark:border-dark-border shadow-sm p-6">
            <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-5 flex items-center">
            ...
            {courses.length === 0 ? (
              <EmptyState
                ...
                description="Enroll in a course to begin your Quran learning journey."
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {courses.map((c) => (
                  <div
                    key={c._id}
                    className="bg-slate-50 dark:bg-dark-bg rounded-xl p-5 border border-slate-200 dark:border-dark-border"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-sm font-bold text-slate-800 dark:text-white">
                        {c.courseTitle || c.courseType}
                      </h3>
                      {badge(c.status)}
                    </div>
                    {c.description && (
          ...
          <div className="bg-white dark:bg-dark-card rounded-xl border border-slate-100 dark:border-dark-border shadow-sm p-6">
            ...
            {upcomingClasses.length === 0 ? (
              <EmptyState
                ...
                description="Your teacher will schedule classes for you. Check back soon!"
              />
            ) : (
              <div className="space-y-3">
                {upcomingClasses.map((cls) => (
                  <div
                    key={cls._id}
                    className="p-4 rounded-xl bg-slate-50 dark:bg-dark-bg border border-slate-200 dark:border-dark-border"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400">
                            {cls.teacherName} &middot; {fmt(cls.scheduledDate)}{" "}
                            at {fmtTime(cls.scheduledDate)} &middot;{" "}
                            {cls.duration}min
                          </p>
                        </div>
                      </div>
      ...
      {activeTab === "history" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <div className="bg-white dark:bg-dark-card rounded-xl border border-slate-100 dark:border-dark-border shadow-sm p-6">
            <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-5 flex items-center">
              <FaHistory className="mr-2 text-slate-500" />
              Class History
            </h2>
            {classHistory.length === 0 ? (
              <EmptyState
                ...
                description="Your completed and past classes will appear here."
              />
            ) : (
              <div className="space-y-3">
                {classHistory.map((cls) => (
                  <div
                    key={cls._id}
                    className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-dark-bg border border-slate-200 dark:border-dark-border"
                  >
                    <div className="flex items-center space-x-4">
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center text-white ${cls.status === "completed" ? "bg-gradient-to-br from-green-500 to-emerald-500" : cls.status === "missed" ? "bg-gradient-to-br from-red-500 to-pink-500" : "bg-gradient-to-br from-slate-400 to-slate-500"}`}
                      >
          ...
          <div className="bg-white dark:bg-dark-card rounded-xl border border-slate-100 dark:border-dark-border shadow-sm p-6">
            ...
            {notifications.length === 0 ? (
              <EmptyState
                ...
                description="You're all caught up! Notifications about your classes will appear here."
              />
            ) : (
              <div className="space-y-2">
                {notifications.map((n) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p
                          className={`text-sm font-medium ${n.read ? "text-slate-600 dark:text-slate-400" : "text-slate-800 dark:text-white"}`}
                        >
                          {n.title}
                        </p>
                        {!n.read && (
                    ...
                    {Boolean(n.metadata?.roomId) &&
                      n.type === "class_scheduled" && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(
                              `/class-room/${String(n.metadata!.roomId)}`,
                            );
                          }}
      ...
      {activeTab === "profile" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <div className="bg-white dark:bg-dark-card rounded-xl border border-slate-100 dark:border-dark-border shadow-sm p-6 max-w-xl">
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-4">
                <div className="w-16 h-16 bg-gradient-to-br from-emerald-500 to-green-600 rounded-2xl flex items-center justify-center text-white text-2xl font-bold">
              ...
              {userInfo?.lastActivity && (
                <p className="text-xs text-slate-400 mt-3">
                  Last active: {fmt(userInfo.lastActivity)}
                </p>
              )}
            </div>
          </div>
